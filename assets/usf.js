/* USF file - Do not modify this file since it is regularly changed. Auto modified at: 11/2/2020 4:56:17 PM*/
/* Impulse 3.3 */
        ...
        <a :href="productUrl" class="grid-product__link " :class="{'grid-product__link--disabled': isSoldOut }" @click="onItemClick" @mouseover="onItemHover" @mouseleave="onItemLeave">
            <div class="grid-product__image-mask">                
                ...
                <div class="image-wrap" :style="'height:0;padding-bottom:' + (selectedImage.height/selectedImage.width*100) + '%'">
                    <img class="grid-product__image lazyload" :data-widths="'[' + _usfImageWidths + ']'" :data-aspectratio="selectedImage.width/selectedImage.height" data-sizes="auto" :alt="product.title" :data-src="_usfGetScaledImageUrl(scaledSelectedImageUrl,'{width}')">
                </div>
                        ...
                        <span class="grid-product__price--savings">
                            ...
                            <template v-else>
                                <template v-if="compareAtPrice && discount">
                                    <span v-html="loc.save"></span> <span>{{salePercent}}%</span>
                                </template>
                            </template>
                        </span>
                    </template>
        ...
        methods: {
            ...
            variantToImg(v) {
                ...
                if (list[v.imageIndex]) {
                    return _usf_getIMGBySizeFromRawURL(list[v.imageIndex].url)
                }
                return list[0] ? _usf_getIMGBySizeFromRawURL(list[0].url) : '';
            }
        },
        render(h) {
...
function _usfGetImageUrls(imageUrl) {
    return _usfImageWidths.map(w => imageUrl.replace('_' + (_usfIsDynamicImage ? '{size}' : usf.settings.search.imageSize), '_' + w).replace("_200x600", "_600x600") + ' ' + w + 'w').join(', ')
}
...
//get origin image with size (for swatchs...etc..)
...
//Ex: product.title = "Hello Word" => handled: hello-word
...
function _usfTruncateWords (str, size = 10, description_words = '...') {
    ...
    if (str.split(' ').length && str.split(' ').length <= size)
        return str;
    return str.slice(0, str.indexOf(' ' + str.split(' ')[size] + ' ')) + description_words
}
...
!function(){var t={props:{loc:Object,variant:Object,args:Object},template:usf.templates.addToCartPlugin};usf.plugins.push({render_searchResultsProductImageExtra(a,r,s,e){var n=s.product,u=s.selectedVariant;if(u||(u=n.variants[0]),!usf.utils.isVariantSoldOut(u))return[r(t,{props:{loc:usf.settings.translation,args:s,variant:u}}),e()]}})}();
